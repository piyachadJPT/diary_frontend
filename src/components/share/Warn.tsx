...
const Warn = ({ advisorId }: AdvisorProps) => {
   ...
   const handleNotificationClick = async (diaryDate: string, id: number, notificationId: number) => {
      try {
         const res = await fetchWithBase(`/api/notification/${notificationId}/read`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
         });

         if (!res.ok) {
            console.error("Failed to mark notification as read:", await res.json());
         }
                  ...
                  {notifications.map((notification, index) => (
                     <React.Fragment key={notification.ID}>
                        <Box
                           ...
                           onClick={() => handleNotificationClick(notification.Data.diary_date, notification.Data.student_id, notification.ID)}
                        >
